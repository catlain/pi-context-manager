...
describe("removeOrphanedToolCalls", () => {
		...
		const messages: any[] = [
			{
				...
				content: [
					...
					},
					{
						...
						id: "call-precision",
						name: "read",
						arguments: '{"path":"/file.ts","offset":10,"limit":5}',
					},
				],
			},
			{
				role: "toolResult",
				toolName: "read",
				toolCallId: "call-coarse",
				content: "very long content here".repeat(500),
			},
			{
				role: "toolResult",
				toolName: "read",
				toolCallId: "call-precision",
				content: "short",
			},
		];

		const filtered = messages.filter(
			(m: any) => m.toolCallId !== "call-coarse",
		);
				...
				content: [
					{
						...
						id: "call-keep",
						name: "read",
						arguments: '{"path":"/file.ts","offset":10}',
					},
					{
						type: "toolCall",
						id: "call-keep2",
	...
	it("assistant 有 text+thinking 时 toolCall 全部移除后保留消息", () => {
		...
		expect(assistant.content).toHaveLength(2);
		expect(assistant.content.every((b: any) => b.type !== "toolCall")).toBe(
			true,
		);
	});
	...
	it("多个 assistant 消息，只删除有孤立 toolCall 的那个", () => {
		...
		expect(
			messages.some(
				(m: any) => m.role === "assistant" && m.content[0]?.id === "call-valid",
			),
		).toBe(true);
	});
});
