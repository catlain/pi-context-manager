...
describe("各工具格式化正确性", () => {
	...
	it("web_search: 空结果", () => {
		expect(formatWebSearchResult(makeWebSearchJson([]))).toBe(
			"搜索结果（共 0 条）",
		);
	});
	...
	it("gh_read_file: 路径+内容", () => {
		const r = formatGhResult(
			makeGhJson({ path: "src/index.ts", content: "console.log('hi')" }),
		);
...
describe("格式化器隔离性", () => {
	...
	it("web_search 输出不被 code-graph 修改", () => {
		const webResult = formatWebSearchResult(
			makeWebSearchJson([
				{
					title: "fn process result",
					link: "https://example.com",
					content: "A function",
				},
			]),
		);
		expect(webResult).not.toBe(
			makeWebSearchJson([
				{ title: "fn process result", link: "https://example.com" },
			]),
		);
	...
	it("web_read 输出不被 code-graph 修改", () => {
		const webResult = formatWebReadResult(
			makeWebReadJson({
				title: "fn processToolResult docs",
	...
	it("gh_search_doc 输出不被 code-graph 修改", () => {
		const ghResult = formatGhResult(
			makeGhJson({
				results: [
					{
						title: "fn processToolResult  core.ts:50",
						url: "https://github.com",
						summary: "class DataStore  store.ts:10",
					},
				],
			}),
		);
	...
	it("含 code-graph 关键词的 JSON 不被格式化", () => {
		const input = JSON.stringify({
			results: [
				{ title: "fn processToolResult  core.ts:50", link: "https://x.com" },
			],
		});
		expect(formatCodeGraphResult(input)).toBe(input);
	});
