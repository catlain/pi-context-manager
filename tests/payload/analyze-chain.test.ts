...
vi.mock("../../payload/core.js", () => {
	const rd = require("node:path").join(
		require("node:os").tmpdir(),
		"pi-payload-analyzer-test-chain",
		"recordings",
	);
	return {
		...
		foldChainEntries: (entries: any[]) => {
			...
			while (i < entries.length) {
				const cur = entries[i];
				const marker =
					({ FULL_KEPT: "📖全文", TRUNCATED: "✂️截断" } as any)[cur.status] ??
					"📝小";
				let j = i + 1;
				while (
					j < entries.length &&
				...
				if (count === 1) {
					lines.push(
						`    req-${cur.req} [${String(cur.idx).padStart(4)}] ${marker} ${cur.status.padEnd(12)} ${String(cur.tokens).padStart(5)}tok ${cur.preview.slice(0, 60)}`,
					);
				} else {
					lines.push(
						`    req-${cur.req}~${entries[j - 1].req} (${count}个) ${marker} ${cur.status.padEnd(12)} ${String(cur.tokens).padStart(5)}tok ${cur.preview.slice(0, 60)}`,
					);
				}
				i = j;
			}
...
vi.mock("../../payload/files.js", async () => {
	...
	const RECORDINGS = require("node:path").join(
		require("node:os").tmpdir(),
		"pi-payload-analyzer-test-chain",
		"recordings",
	);
					...
					for (const m of msgs) {
						...
						for (const pm of msgs) {
							if (pm.role === "assistant" && pm.tool_calls) {
								for (const tc of pm.tool_calls) {
									if (tc.id === tcid) toolName = tc.function.name;
								}
							}
						}
						...
						timeline.get(sig).push({
							req: reqNum,
							idx: 0,
							status: text.includes("[processed]")
								? "TRUNCATED"
								: text.length / 4 >= 500
									? "FULL_KEPT"
									: "SMALL",
...
beforeEach(() => {
	mkdirSync(RECORDINGS_TMP, { recursive: true });
});
afterEach(() => {
			...
			messages: [
				{
					role: "assistant",
					tool_calls: [
						{ id: "tc-other", function: { name: "read", arguments: "{}" } },
					],
				},
