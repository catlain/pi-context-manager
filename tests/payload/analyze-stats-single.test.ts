...
vi.mock("../../payload/core.js", () => {
	const rd = require("node:path").join(
		...
		"pi-payload-analyzer-test-stats-single",
		"recordings",
	);
	return {
	...
	return {
		getRecordingFiles: () => {
			...
			const e = fs
				...
				.sort()
				.map((f: string) => ({ filename: f, path: path.join(RECORDINGS, f) }));
		...
		writeReq("test-distill.json", {
			messages: [
				...
				},
				{
					...
					tool_call_id: "tc1",
					content:
						"[distilled read] some/path Original: ~5000 tokens\nPreview...",
				},
			],
		});
		expect(doSingle(join(RECORDINGS_TMP, "test-distill.json"))).toContain(
			"SMALL",
		);
	});
