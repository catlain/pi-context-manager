...
function msg(role: string, content: string) { return { role, content }; }
...
describe("doMessages — grep 按关键词搜索", () => {
	...
	it("搜索 'main.ts' 命中相关消息", () => {
		const result = doMessages({ payloadPath: "/tmp/test.json", grep: "main.ts" });
	...
	it("搜索不存在的关键词返回空结果提示", () => {
		const result = doMessages({ payloadPath: "/tmp/test.json", grep: "zzz_nonexistent" });
	...
	it("grep 搜索 tool call 的 arguments", () => {
		const result = doMessages({ payloadPath: "/tmp/test.json", grep: "test.ts" });
		hasIdx(result, 6); // assistant tool_call 的 arguments 含 test.ts
		noIdx(result, 7); // tool result content 中不含 test.ts
	});

	it("grep 支持正则表达式", () => {
		const result = doMessages({ payloadPath: "/tmp/test.json", grep: "main\\.ts|test\\.ts" });
		expect(result).toContain("main.ts");
...
describe("doMessages — toolName 按工具名过滤", () => {
	...
	it("工具名不存在时返回空结果", () => {
		const result = doMessages({ payloadPath: "/tmp/test.json", toolName: "nonexistent_tool" });
		expect(result).toContain("没有匹配");
	});
});
...
describe("doMessages — 组合参数和边缘情况", () => {
	...
	it("msgIndex + msgRange 同时传时优先用 msgIndex", () => {
		mockReadJsonFile.mockReturnValue(makePayload([msg("user", "hi"), msg("assistant", "hello")]));
		const result = doMessages({ payloadPath: "/tmp/test.json", msgIndex: 0, msgRange: "0-1" });
		hasIdx(result, 0);
	...
	it("grep + toolName 同时传时取交集", () => {
		...
		const result = doMessages({ payloadPath: "/tmp/test.json", grep: "MATCH", toolName: "read" });
		hasIdx(result, 1);
		noIdx(result, 3);
		noIdx(result, 5);
	});
	...
	it("单条消息的 payload + msgRange last:1 正常", () => {
		mockReadJsonFile.mockReturnValue(makePayload([msg("user", "only one message")]));
		const result = doMessages({ payloadPath: "/tmp/test.json", msgRange: "last:1" });
		hasIdx(result, 0);
		expect(result).toContain("only one message");
	});

	it("content 为数字不崩溃", () => {
		mockReadJsonFile.mockReturnValue(makePayload([{ role: "user", content: 42 }]));
		const result = doMessages({ payloadPath: "/tmp/test.json" });
		hasIdx(result, 0);
