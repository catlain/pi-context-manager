...
export function collectData(
	...
	for (const m of msgs) {
		if (m.role === "user") {
			...
			asstMsgs.push({
				summary:
					textParts.join("").split("\n")[0].slice(0, 60) || "(tool calls)",
				callTokens: txtSz,
				resultTokens: 0,
			...
			const matched =
				linkedRecs?.find((r) => r.resultTokens === 0) ||
				b.records.find((r) => r.resultTokens === 0);
			if (matched) {
				...
				const rec: RecordItem = {
					summary: rLines[0]?.slice(0, 60) || "(result)",
					callTokens: 0,
					resultTokens: rs,
					lines: rLines,
	...
	const ratio =
		sysRaw + toolDefTokens + msgRaw + callRaw + resultRaw > 0
			? total / (sysRaw + toolDefTokens + msgRaw + callRaw + resultRaw)
			: 1;
	...
	const cats: CategoryItem[] = [
			...
			children: [
				{
					...
					records: [
						{
							summary: `${sysLines.length} lines`,
							callTokens: cal(sysRaw),
							resultTokens: 0,
							lines: sysLines,
						},
					],
		...
		{
			label: "System Tools",
			value: cal(toolDefTokens),
			color: "dim",
			enterable: toolChildren.length > 0,
		...
		},
		{
			...
			children: [
				{
					label: "User",
					value: cal(userMsgs.reduce((s, r) => s + r.callTokens, 0)),
					callTokens: 0,
					resultTokens: 0,
					color: "accent",
					enterable: userMsgs.length > 0,
					records: [...userMsgs].reverse(),
				},
				{
					label: "Assistant",
					value: cal(asstMsgs.reduce((s, r) => s + r.callTokens, 0)),
					callTokens: 0,
					resultTokens: 0,
					color: "accent",
					enterable: asstMsgs.length > 0,
					records: [...asstMsgs].reverse(),
				},
				{
					label: "Summaries",
					value: 0,
				...
				.map(([n, v]) => {
					...
					if (manuallyDeletedIds.size > 0) {
						for (const r of v.records) {
							if (r.toolCallId && manuallyDeletedIds.has(r.toolCallId))
								r.manuallyDeleted = true;
						}
					}
					return {
						label: n,
						value: cal(v.callT + v.resultT),
						callTokens: cal(v.callT),
						resultTokens: cal(v.resultT),
						color: "success",
						enterable: v.records.length > 0,
						records: v.records,
					};
				})
				.sort((a, b) => b.value - a.value),
	...
	if (total - accounted > 10)
		cats.push({
			label: "Available",
			value: Math.max(0, total - accounted),
			color: "dim",
			enterable: false,
			children: [],
		});
