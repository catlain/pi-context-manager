...
import { listRecordings, listSessions } from "./files-core.js";
...
export function doList(sessionId?: string): string {
	...
	if (sessions.length > 0) {
		...
		for (const s of sessions) {
			...
			lines.push(`\n  📁 ${s.sessionId}${mark}`);
			lines.push(
				`     ${s.fileCount} 文件  ${(s.totalSize / 1024).toFixed(0)}KB  ${s.model}  ${s.firstTs}~${s.lastTs}`,
			);
			...
			if (s.sessionId === sessionId || (!sessionId && sessions.length === 1)) {
				...
				for (const f of files) {
					lines.push(
						`     ${f.filename.padEnd(40)}  ${(f.size / 1024).toFixed(1).padStart(6)}KB  ${f.msgCount} msgs  ${f.model}`,
					);
				}
			}
		}
	...
	for (const { filename, path } of files) {
		...
		lines.push(
			`  ${filename.padEnd(40)}  ${(size / 1024).toFixed(1).padStart(6)}KB  ${msgCount} msgs  ${model}`,
		);
	}
	return lines.join("\n");
}
...
export function doSingle(payloadPath: string, threshold = 500): string {
	...
	for (const r of results.slice(-8)) {
		lines.push(
			`  [${String(r.idx).padStart(4)}] ${r.tool.padEnd(20)} ${r.status.padEnd(12)} ${String(r.tokens).padStart(5)}tok`,
		);
	}
	return lines.join("\n");
}
...
export function doChain(sessionId?: string): string {
	...
	const chains = [...timeline.entries()]
		...
		.sort(
			(a, b) =>
				Math.max(...b[1].map((e) => e.tokens)) -
				Math.max(...a[1].map((e) => e.tokens)),
		);
	...
	for (const [sig, entries] of chains) {
		...
		if (statuses.includes("FULL_KEPT")) {
			...
			if (fulls.length === entries.length) {
				...
				lines.push(
					`    ✅ 首次保留(req-${fulls[fulls.length - 1].req}) → 后续正确消失`,
				);
			}
		}
	}
