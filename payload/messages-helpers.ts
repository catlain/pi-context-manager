...
/** 消息行摘要 */
export function summaryLine(i: number, m: any, toolIdx: Map<string, any>): string {
	const role = m.role ?? "?";
...
/** 消息详情（用于 msgIndex 模式） */
export function detailBlock(i: number, m: any, toolIdx: Map<string, any>): string {
	const role = m.role ?? "?";
	const text = getText(m.content);
	const tokens = estTokens(text);
...
export function searchableText(m: any): string {
