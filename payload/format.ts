 ...
 */
export function foldChainEntries(
	entries: Array<{
		req: string;
		idx: number;
		status: string;
...
export function formatToolStats(
	...
	const sorted = Object.entries(perTool).sort(
		(a, b) =>
			b[1].callTokens +
			b[1].resultTokens -
			(a[1].callTokens + a[1].resultTokens),
	);
	for (const [name, s] of sorted) {
		const total = s.callTokens + s.resultTokens;
		lines.push(
			`   ${name.padEnd(25)} ${String(s.count).padStart(5)} ${fmtTok(s.callTokens).padStart(8)} ${fmtTok(s.resultTokens).padStart(8)} ${fmtTok(total).padStart(8)}`,
		);
	}
	return lines.join("\n");
}
