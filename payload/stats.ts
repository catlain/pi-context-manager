...
export function doStats(sessionId?: string): string {
	...
	for (const s of ["TRUNCATED", "FULL_KEPT", "SMALL"]) {
		...
		lines.push(
			`  ${s.padEnd(20)}: ${String(counts[s] ?? 0).padStart(4)} (${pct}%)`,
		);
	}
	return lines.join("\n");
}
...
export function doChainTcId(sessionId?: string): string {
	...
	for (const [tcid, entries] of chains) {
		...
		if (kept.length === entries.length) {
			lines2.push(
				"    ⚠️  全部 FULL_KEPT — distill 未生效（旧 tcId 未被静默删除）",
			);
		} else if (kept.length > 0) {
			lines2.push(
				`    ✅ 首次保留(req-${kept[0].req}) → 后续 ${entries.length - kept.length} 次正确消失`,
			);
		} else {
			lines2.push(
				`    ⚠️  无 FULL_KEPT 出现 — tcId 在所有 payload 中都被截断/删除`,
			);
		}
	}
	return lines2.join("\n");
