...
export default function registerContextCommand(
	pi: ExtensionAPI,
	stateRef: ContextStateRef,
) {
			...
			await ctx.ui.custom(
				(tui, theme, kb, done) => {
					...
					const render = () => {
						switch (lvl) {
							case "overview":
								renderOverview(container, data, theme, oIdx);
								break;
							case "category":
								renderCategory(container, data, theme, curCat, dIdx);
								break;
							case "records":
								renderRecords(
									container,
							...
							case "content":
								if (curRecord)
									renderContent(
										container,
										theme,
										`${curCat.label} › ${curDetail?.label} › #${rIdx + 1} ${curRecord.summary.slice(0, 30)}`,
										curRecord,
										scroll,
										viewport,
										confirmingDelete,
						...
						handleInput: (kd: unknown) => {
							...
							if (confirmingDelete) {
								if (keyY && curRecord?.toolCallId) {
									...
									ctx.ui.notify(
										`已标记删除: ${curRecord.summary.slice(0, 40)}`,
										"info",
									);
									render();
									return;
							...
							if (lvl === "content" && curRecord) {
								...
								if (up) scroll = Math.max(0, scroll - 1);
									...
									keyD &&
									curRecord.toolCallId &&
									!curRecord.manuallyDeleted
								) {
							...
							if (up || dn) {
								const dir = up ? -1 : 1;
								if (lvl === "overview")
									oIdx = clamp(oIdx + dir, 0, data.categories.length - 1);
								else if (lvl === "category")
									dIdx = clamp(
										dIdx + dir,
										0,
										(curCat.children?.length ?? 1) - 1,
									);
								else if (lvl === "records") {
									rIdx = clamp(
										rIdx + dir,
									...
									else if (rIdx >= rScroll + visible)
										rScroll = rIdx - visible + 1;
								}
								render();
								return;
							}
							...
							if (ok) {
								if (lvl === "overview") {
									...
									if (cat?.enterable && cat.children?.length) {
										curCat = cat;
										dIdx = 0;
										lvl = "category";
									}
									...
									if (ch?.enterable && ch.records?.length) {
										curDetail = ch;
										rIdx = 0;
										rScroll = 0;
										lvl = "records";
