...
describe("buildArgsSignature", () => {
	it("read 工具用 path", () => {
		expect(buildArgsSignature("read", { path: "/home/user/file.ts" })).toBe(
			"/home/user/file.ts",
		);
	});

	it("edit 工具用 path", () => {
		expect(buildArgsSignature("edit", { path: "src/index.ts" })).toBe(
			"src/index.ts",
		);
	});
	...
	it("bash 工具用 command 第一行前 80 字符", () => {
		...
		expect(buildArgsSignature("bash", { command: longCmd })).toBe(
			"echo 'a'.repeat(1000)",
		);
	...
	it("grep 工具用 pattern in path", () => {
		expect(buildArgsSignature("grep", { pattern: "TODO", path: "src/" })).toBe(
			"TODO in src/",
		);
...
describe("formatTmpContent", () => {
	it("包含 [distilled toolName] 标记", () => {
		const content = formatTmpContent(
			{ name: "read", meta: "/file.ts" },
			["line1", "line2"],
			5000,
		);
...
describe("buildArgsSignature — 同路径返回同签名", () => {
	it("read 同路径（无 offset/limit vs 有 offset/limit）返回同签名", () => {
		...
		const sig2 = buildArgsSignature("read", {
	...
	it("read 同路径不同 offset 返回同签名", () => {
		const sig1 = buildArgsSignature("read", {
			path: "/home/user/file.ts",
			offset: 10,
			limit: 5,
		});
